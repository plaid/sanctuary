...
(f => {
  ...
  const get_ = key => (
    B (obj => key in obj ? Just (obj[key]) : Nothing) (toObject)
  );
  ...
  //. Nothing
  //. ```
  const lift3 = f => applyX => applyY => applyZ => (
  ...
  //. Just ([1, 2, 3])
  //. ```
  const parseJson = pred => (
    B (filter (pred)) (B (either (K (Nothing)) (Just)) (encase (JSON.parse)))
  );
  _.parseJson = {
    consts: {},
